...
var decode = function(msg) {
    ...
    var route = msg.route;
    var app = this.app || this.protobuf.app;
    ...
    var decryped = msg.body;
    if (!!app.encrypt){
        decryped = app.encrypt.decodeMessage(msg.body);
    }
...
var composeResponse = function(server, msgId, route, msgBody) {
    ...
    var crypted = msgBody;
    if (!!server.protobuf.app.encrypt){
        crypted = server.protobuf.app.encrypt.encodeMessage(msgBody);
    }
...
var emptyBuffer = new Buffer(JSON.stringify({}), 'utf8');
...
var encodeBody = function(server, route, msgBody) {
    ...
    if(!!server.protobuf && !!server.protobuf.getProtos().server[route]) {
     ...
     if(msgBody == null){
      //logger.debug('encodeBody msgBody is null', route);
      return emptyBuffer;
    }
