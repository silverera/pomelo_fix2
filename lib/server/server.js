  ...
  try {
    app.sysrpc[routeRecord.serverType].msgRemote.forwardMessage(
      ...
      function(err, resp, opts) {
        if(err) {
          logger.error('fail to process remote message msg:%j, routeRecord:%j', msg, routeRecord);
    ...
    if(!finished) {
      logger.error('fail to forward message msg:%j, routeRecord:%j', msg, routeRecord);
