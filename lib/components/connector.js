...
var handleMessage = function (self, session, msg) {
    if (msg.route.indexOf('uploadCharState')<0)
    ...
    if (msg.id){
        ...
        if(msg.id < expect) {
            logger.warn('requestID invalid, potential cheating. route:%s,ip:%s,account:%s,expectID:%d',
                msg.route, session.__socket__.remoteAddress.ip, session.get('account'), expect);
            return;
        }
        session.set('nextReqID', expect + 1);
    }

    //var packetCache = session.get('packetCache');
    //if (msg.route == "connector.entryHandler.packetAck") {
    //    var found = false;
    //    for(var i=0;i<packetCache.length;i++){
    //        var e = packetCache[i];
    //        if (e.msgID == msg.body.msgID && e.msgType == msg.body.msgType){
    //            packetCache.splice(i, 1);
    //            found = true;
    //            break;
    //        }
    //    }
    //    if (!found)
    //        logger.debug('ack msg not in cache:%d,type=%d', msg.body.msgID, msg.body.msgType);
    //    return;
    //}
    ...
    self.server.globalHandle(msg, session.toFrontendSession(), function (err, resp, opts) {
        ...
        if (__pushMsgs) {
            for (var id in __pushMsgs) {
                ...
                //if(!pushMsg.noAck){
                //    pushMsg.msgID = session.get('nextPushMsgID');
                //    packetCache.push({
                //        msgID: pushMsg.msgID,
                //        msgType: 3, //push
                //        route: pushMsg.route,
                //        body: pushMsg.resp
                //    });
                //    session.set('nextPushMsgID', pushMsg.msgID+1);
                //}
