...
var handleMessage = function (self, session, msg) {
    ...
    if (msg.id != session.get('nextMsgID')) {
        logger.warn('duplicated msg from client. potential cheating. ip:%s,account:%s', session.__socket__.remoteAddress.ip, session.get('account'));
        return;
    }
    session.set('nextMsgID', session.get('nextMsgID') + 1);
    ...
    if (msg.route == "connector.entryHandler.packetAck") {
        ...
        // character sync data doest need to ack..
        if (msg.body.needAck) {
